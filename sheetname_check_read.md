...
When data pipeline requires a user-uploaded excel file to be read to a database,  a common breakpoint occurs when the ETL process fails spectacularly :fearful: because the 
the excel sheetnames do not match the default set-up. Here's some base code for programmatically checking that sheetnames  are correct before trying to load to DB. 

See fullcode [here](https://github.com/ShanNatRichards/Csharp/blob/main/sheetname_check.cs).
